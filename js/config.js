...
// IMPORTANTE: este arquivo precisa carregar antes do js/app.js no index.html.
...
if (!window.supabase) {
  console.error('Biblioteca Supabase não foi carregada antes do config.js.');
} else {
  window.bncSupabase = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
  console.log('✅ Supabase BNC inicializado.');
}
