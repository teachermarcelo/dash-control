...
async function handleLogin(e) {
    ...
    try {
        ...
        const { data: profile } = await supabase.from('bnc_profiles').select('role, full_name, region').eq('id', currentUser.id).single();
...
async function loadDashboardData() {
    ...
    document.getElementById('kpi-contracts').textContent = totalContracts;
    document.getElementById('kpi-employees').textContent = totalEmployees;
    document.getElementById('kpi-tasks').textContent = `${completed} / ${tasks?.length || 0}`;
...
function switchView(id) {
    ...
    document.querySelectorAll('.mobile-nav-item').forEach(e => {
        e.classList.remove('active','text-brand'); e.classList.add('text-gray-400');
        if(e.getAttribute('onclick')?.includes(id)) { e.classList.add('active','text-brand'); e.classList.remove('text-gray-400'); }
    });
}
